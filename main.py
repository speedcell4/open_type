...
from model_utils import get_eval_string, get_gold_pred_str, get_output_index
    ...
    if args.mode == 'train':
        if not args.remove_open and not args.only_crowd:
            ...
            valid_gen_list.append(
                ("open", get_data_gen('distant_supervision/headword_dev.json', 'dev', args, vocab, "open")))
        if not args.remove_el and not args.only_crowd:
            valid_gen_list.append(
                ("wiki",
                 get_data_gen('distant_supervision/el_dev.json', 'dev', args, vocab,
                              "wiki" if args.multitask else "open")))
            train_gen_list.append(
                ("wiki",
                 get_data_gen('distant_supervision/el_train.json', 'train', args, vocab,
                              "wiki" if args.multitask else "open")))
...
def _train(args):
    ...
    while True:
        ...
        if batch_num % args.save_period == 0 and batch_num > 0:
            ...
            print(
                'Total {0:.2f} minutes have passed, saving at {1:s} '.format((time.time() - init_time) / 60,
                                                                             save_fname))
    # Training finished!
    torch.save({'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()},
               '{0:s}/{1:s}.pt'.format(constant.EXP_ROOT, args.model_id))
...
if __name__ == '__main__':
    ...
    logging.basicConfig(
        filename=constant.EXP_ROOT + "/" + config.model_id + datetime.datetime.now().strftime(
            "_%m-%d_%H") + config.mode + '.txt',
